              ...
              return (
                <li key={item.name} className="flex flex-col relative">
                  <div
                    ...
                    className={`cursor-pointer flex items-center justify-between rounded-lg transition-all duration-200 ${isOpen ? 'px-4 py-3' : 'justify-center py-3'
                      } ${isItemActive
                      ...
                      {item.subItems.map((sub) => {
                        ...
                        return (
                          <li key={sub.name} className="relative group mt-1">
                            {/* Circle Node */}
                            <div className={`absolute left-[22px] top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full border-2 bg-[#0B1120] z-10 transition-colors ${isSubActive ? 'border-brand-bright-orange' : 'border-[#475569] group-hover:border-slate-400'
                              }`}></div>

                            <Link
                              href={sub.href}
                              className={`block pl-[44px] py-2 text-[15px] font-medium transition-colors ${isSubActive ? 'text-brand-bright-orange' : 'text-slate-300 hover:text-white'
                                }`}
                            >
                              {sub.name}
                            </Link>
